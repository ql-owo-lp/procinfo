...
gva_t findMMStructFromTaskStruct(gva_t ts, uint32_t* pMMStructOffset, uint32_t* pPGDOffset, int bDoubleCheck)
{
  uint32_t i = 0;
  gva_t* temp = NULL;
...
int isListHead(gva_t lh)
{
  ...
  if ( !isKernelAddress((gva_t)temp) || !isKernelAddress((gva_t)(*temp))
     || !isKernelAddress((gva_t)temp2) || !isKernelAddress((gva_t)(*temp2))
     )
  {
    return (0);
  }
...
gva_t findTaskStructListFromTaskStruct(gva_t ts, uint32_t mmStructOffset, uint32_t* pTaskStructListOffset, int bDoubleCheck)
{
  uint32_t i = 0;
  gva_t* temp = NULL;
...
gva_t findTaskStructFromThreadInfo(gva_t threadinfo, uint32_t* pTaskStructOffset, uint32_t* pThreadInfoOffset, int bDoubleCheck)
{
  int bFound = 0;
  uint32_t i = 0;
  uint32_t j = 0;
...
gva_t findRealParentGroupLeaderFromTaskStruct(gva_t ts, ProcInfo* pPI)
{
  ...
  for (i = 0; i < MAX_TASK_STRUCT_SEARCH_SIZE; i+=4)
  {
    if ( isTaskStruct(*((gva_t*)(ts+i)), pPI) //real_parent
       ...
       && isTaskStruct(*((gva_t*)(ts+i+4+4+SIZEOF_LIST_HEAD+SIZEOF_LIST_HEAD)), pPI) //group_leader
       )
    {
      ...
      {
        pPI->ts_group_leader = i+4+4+SIZEOF_LIST_HEAD+SIZEOF_LIST_HEAD;
      }
      return (ts+i);
    }
  }
...
gva_t findCredFromTaskStruct(gva_t ts, ProcInfo* pPI)
{
  ...
  for (i = 16; i < pPI->ts_comm; i+=4)
  {
    if ( isListHead(GET_FIELD(ts, pPI->ts_comm - i))
        && isKernelAddress(GET_FIELD(ts, pPI->ts_comm - i + SIZEOF_LIST_HEAD))
        && isKernelAddress(GET_FIELD(ts, pPI->ts_comm - i + SIZEOF_LIST_HEAD + 4))
       )
    {
...
int try_vmi(void)
{
  ...
  taskstruct = findTaskStructFromThreadInfo(threadinfo, &pi.ti_task, &pi.ts_stack, 0); 
